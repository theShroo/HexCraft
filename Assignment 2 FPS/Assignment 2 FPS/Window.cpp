...
#include "Window.h"
#include <iostream>
Window* Window::g_window = 0;

Window::Window(LPCWSTR windowName, int width, int height, bool fullscreen)
{
...
void Window::Start()
{
	...
	while (m_running)
	{
		...
		// (while gives better responsivness to the controls)
		...
		if (m_running) {
			...
			if (currentCount.QuadPart - m_lastFrame.QuadPart / (float)m_counterFrequency.QuadPart > targetTime) {
				m_game->Render();			//After everything is updated we can then render a frame
				m_lastFrame = currentCount;
...
LRESULT CALLBACK Window::MessageProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_KEYDOWN:
		m_input->SetKeyDown(UINT (wParam));
		return 0;

	case WM_KEYUP:
		m_input->SetKeyUp(UINT(wParam));
		return 0;
	...
	case WM_INPUT:
	{	
		// problem area here, 64 bit uses a size = 48, while x86 uses size = 40.
		UINT dwSize = 48;
		static BYTE lpb[48];
