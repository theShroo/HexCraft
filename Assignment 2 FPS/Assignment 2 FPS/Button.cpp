...
Button::Button(int width, int height, Texture* texture, LPCWSTR text, XMFLOAT2 position, 
	SpriteBatch* spriteBatch, SpriteFont* font, InputController* input, std::function<void()> onClickFunction)
{
	...
	m_text = text;
	m_position = position;
	m_textColour = XMFLOAT4(0.2f, 0.2f, 0.2f, 1.0f);
	m_isEnabled = true;
	...
	m_origin = XMFLOAT2(m_width / 2.0f, m_height / 2.0f);	//The first value to calculate is the origin, this defines what part of the texture will be placed at m_position
	...
	XMVECTOR stringSize = m_font->MeasureString(m_text);	//The final value to calculate is the origin of the text. To do this we need to know how big
	XMFLOAT2 temp;
	XMStoreFloat2(&temp, stringSize);
	m_textOrigin = XMFLOAT2(temp.x / 2, temp.y / 2);	//the string will be when it's rendered with our font. We then divide this by 2 to get an
	...
	m_onClickFunction_1 = 0;
}

Button::Button(int width, int height, Texture* texture, LPCWSTR text, XMFLOAT2 position,
	SpriteBatch* spriteBatch, SpriteFont* font, InputController* input, std::function<void(int a)> onClickFunction, int index)
{
	//Setting basic values
	m_width = width;
	m_height = height;
	...
	m_onClickFunction = 0;													//We also need to make sure we remember our function object
	m_onClickFunction_1 = onClickFunction;
	m_index = index;
}
		...
		m_input->GetMouseY() > (m_position.y - halfHeight) && m_input->GetMouseY() < (m_position.y + halfHeight))
	{
		...
		if (m_isEnabled)
		{
			...
			if (m_input->GetMouseUp(LEFT_MOUSE)) {
				...
				else if (m_onClickFunction_1) {
					m_onClickFunction_1(m_index);
				}
			}
		}		
	}
...
void Button::Render()
{
	...
	m_spriteBatch->Draw(m_buttonTexture->GetShaderResourceView(), MathsHelper::GetXMVECTOR2(m_position), &m_textureWindow, MathsHelper::GetXMVECTOR4(1.0f, 1.0f, 1.0f, alpha), 0, MathsHelper::GetXMVECTOR2(m_origin));

	//Then we draw the text at our position, using the origin we calculated and the colour that was set
	m_font->DrawString(m_spriteBatch, m_text, MathsHelper::GetXMVECTOR2(m_position), MathsHelper::GetXMVECTOR4(m_textColour), 0, MathsHelper::GetXMVECTOR2(m_textOrigin));
}